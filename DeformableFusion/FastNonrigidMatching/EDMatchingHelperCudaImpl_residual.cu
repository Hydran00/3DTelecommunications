...
__global__ void evaluate_per_vertex_residual_kernel(float *dev_per_vertex_align_residual,
...
__global__ void evaluate_per_vertex_residual_kernel_vVolume(float *__restrict__ dev_per_vertex_align_residual,
...
#pragma unroll
			...
			{
				float hull_val = tex3D<float>(tex_visHull, x, y, z);
...
__global__ void aggregate_ed_nodes_residual_kernel_v2(float *__restrict__ dev_ed_nodes_align_residual,
...
// one ed nodes per cuda block
__global__ void aggregate_ed_nodes_residual_kernel(float *dev_ed_nodes_align_residual,
...
__global__ void ed_nodes_reg_residual_check_kernel(float *dev_ed_nodes_align_residual,
...
__global__ void interp_per_vertex_residual_from_ed_kernel(float *dev_per_vertex_align_residual_ed_interp,
...
__global__ void calc_depth_align_residual_kernel(float *dev_depth_align_residual_,
												 ...
												 int depth_width_ori, int depth_height_ori, float residual_cap)
{
	...
	if (i < dev_num_cam_views * depth_width_out * depth_height_out)
	{
		...
		unsigned short d = tex2DLayered<unsigned short>(tex_depthImgs, (float)u_ori, (float)v_ori, vId);
		depth_extract_fg(d);
...
__global__ void calc_depth_align_residual_kernel_vVolume(float *dev_depth_align_residual_,
														 ...
														 float cube_res, float vxl_res, int cube_size_in_vxl, int vxls_per_cube)
{
	...
	if (i < dev_num_cam_views * depth_width_out * depth_height_out)
	{
		...
		if (d > 0)
		{
			...
			float4 nd_4d = tex2DLayered<float4>(tex_normalMaps, (float)u, (float)v, vId);
...
__global__ void depth_align_residual_filter_kernel(float const *dev_depth_align_residual_in,
