...
__global__ void project_points_to_all_depth_maps_kernel(int *dev_depth_maps_proj, int depth_width_out, int depth_height_out,
														int depth_width_ori, int depth_height_ori, float vxl_res,
...
__global__ void record_front_vtIdx_all_depth_maps_kernel(int *dev_depth_mats_corr_vtIdx, int const *__restrict__ dev_depth_maps_proj, int depth_width_out, int depth_height_out,
														 int depth_width_ori, int depth_height_ori, float vxl_res,
														 float const *__restrict__ dev_vts, int const *dev_vts_num, int vts_num_max, int vt_dim)
{
	const int vts_num = MIN(vts_num_max, *dev_vts_num);
...
#ifndef THREADS_PER_BLOCK_COSTKERNEL
...
__global__ void ComputeJtJDataOffDiagonalTermKernel_SM2(float *dev_jtj, HessianBlockInfoCuda const *__restrict__ dev_jtj_blk_info,
...
// shared memory version for JitJj
__global__
	__launch_bounds__(160, 6) void ComputeJtJDataOffDiagonalTermKernel_SM(float *__restrict__ dev_jtj, HessianBlockInfoCuda *__restrict__ dev_jtj_blk_info,
																		  float const *__restrict__ dev_vts, float const *__restrict__ dev_vts_t, int vt_dim,
																		  DeformGraphNodeCuda const *__restrict__ dev_ed_nodes, RigidTransformCuda const *__restrict__ dev_rigid_transf,
																		  int const *__restrict__ dev_vt_indices, float const *__restrict__ dev_node_weights,
																		  ushort2 const *__restrict__ dev_cam_vis,
...
__global__ void ComputeJtJDataDiagonalTermKernel_SM(float *dev_jtj, HessianBlockInfoCuda *dev_jtj_blk_info,
													float const *__restrict__ dev_vts, float const *__restrict__ dev_vts_t, int vt_dim,
													DeformGraphNodeCuda const *__restrict__ dev_ed_nodes, RigidTransformCuda const *__restrict__ dev_rigid_transf,
													int const *__restrict__ dev_vt_indices, float const *__restrict__ dev_node_weights,
													ushort2 *dev_cam_vis,
...
__global__
	__launch_bounds__(160, 6) void ComputeJtFDataTermKernel_SM(float *__restrict__ dev_jtf, HessianBlockInfoCuda const *__restrict__ dev_jtj_blk_info,
															   float const *__restrict__ dev_vts, float const *__restrict__ dev_vts_t, int vt_dim,
															   DeformGraphNodeCuda const *__restrict__ dev_ed_nodes, RigidTransformCuda const *__restrict__ dev_rigid_transf,
															   int const *__restrict__ dev_vt_indices, float const *__restrict__ dev_node_weights,
															   ushort2 const *__restrict__ dev_cam_vis,
...
__global__ void set_gpu_costs_to_zero_kernel(float *dev_cost1,
											 float *dev_cost2,
											 float *dev_cost3,
											 float *dev_cost4,
											 float *dev_cost5,
...
__global__ void sum_over_gpu_costs_kernel(float *dev_cost_sum,
...
bool EDMatchingHelperCudaImpl::allocate_and_texBind_cuda_arrays_depth(int tex_num, int width, int height)
{
	...
	// create texture object bound to array
	{
		...
		checkCudaErrors(cudaMemcpyToSymbol(tex_depthImgs_dev, &tex_depthImgs, sizeof(cudaTextureObject_t)));
	}
...
void EDMatchingHelperCudaImpl::bind_cuda_array_to_texture_normal(cudaArray *cu_3dArr_normal)
{
	...
	if (tex_normalMaps)
	{
		cudaDestroyTextureObject(tex_normalMaps);
		tex_normalMaps = 0;
	}
	{
		cudaResourceDesc resDesc = {};
		resDesc.resType = cudaResourceTypeArray;
		resDesc.res.array.array = cu_3dArr_normal;
		cudaTextureDesc texDesc = {};
		texDesc.addressMode[0] = cudaAddressModeClamp;
		texDesc.addressMode[1] = cudaAddressModeClamp;
		texDesc.filterMode = cudaFilterModePoint;
		...
		checkCudaErrors(cudaMemcpyToSymbol(tex_normalMaps_dev, &tex_normalMaps, sizeof(cudaTextureObject_t)));
	}
}
