...
template <int vtStep>
__global__ void evaluate_cost_rigid_alignment_kernel(float *dev_cost_global_rigid,
...
__global__ void rodrigues_kernel(float const *dev_rod, float *dev_R, float *dev_dR_drod)
{
...
template <int vtStep>
__global__ void evaluate_jtj_jtf_rigid_align_kernel(float *__restrict__ dev_partial_JtJs, float *__restrict__ dev_partial_JtFs,
													float const *dev_R, float const *dev_T,
...
__global__ void jtj_rigid_align_reduction(float *dev_JtJ, float const *dev_partial_JtJs, int partial_JtJs_count)
{
...
__global__ void jtf_rigid_align_reduction(float *dev_JtF, float const *dev_partial_JtFs, int partial_JtFs_count)
{
	__shared__ float vals[MAX_THREADS_PER_BLOCK];
...
__global__ void apply_rigid_alignment_to_RigidTransf_kernel(float const *dev_R, float const *dev_T, RigidTransformCuda *dev_rigid_transf)
{
...
__global__ void solve_and_update_R_T_rigid_align_kernel(float *dev_rod, float *dev_T, float const *dev_JtJ, float const *dev_JtF)
{
...
// use only one cuda block with 36 threads
__global__ void solve_rigid_align_kernel(float *dev_rod_dx, float *dev_T_dx, float const *dev_JtJ, float const *dev_JtF, float opt_mu)
{
#define JTJ_BLOCK_DIM 6
	__shared__ float sh_L[JTJ_BLOCK_DIM * JTJ_BLOCK_DIM];
	__shared__ float sh_x[JTJ_BLOCK_DIM];
